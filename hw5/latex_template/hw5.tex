...
\newcommand{\hwTopic}{Neural Networks}
\newcommand{\hwName}{\hwNum: \hwTopic}
\newcommand{\outDate}{Tuesday, February 27}
\newcommand{\dueDate}{Sunday, March 16}
\newcommand{\taNames}{Doris, Jenny, Max, Siyan, Zoe, Markov, Neural}
\newcommand{\homeworktype}{\string written+prog}
\newcommand{\exitPollLink}{\string https://forms.gle/AMHwEUiQhjv1kbHu9}
...
$$\boldsymbol{\beta}=
    ...
    \begin{subparts}
        ...
        \choice Prepend a value of 1 to be the first entry of $\zv$.
        \choice Prepend an additional column of 1's to be the first column of $\boldsymbol{\beta}$ 
        \choice Prepend a row of 1's to be the first row of $\boldsymbol{\beta}$ 
        \end{checkboxes}
        ...
        % YOUR ANSWER
        % Change \choice to \CorrectChoice for the appropriate selection/selections 
        \choice $\betav$ with just one column with non-zeros values
        \choice $\betav$ with just one row with non-zeros values
        \choice $\betav$ with all weights = 1
        \choice $\betav$ with all weights = 0
        \choice $\betav$ where: 1) $\betav_{ii}=1$ and 2) $\betav_{ij}=0$ where $i!=j$
        \end{checkboxes}
...
\begin{subparts}
    ...
    \begin{checkboxes}
        % YOUR ANSWER
        % Change \choice to \CorrectChoice for the appropriate selection/selections 
        \choice The bias terms do not update, so there is no need to include them in backpropagation.
        \choice It is the computationally cheapest column to remove to ensure that the dimensions match.
        \choice The elements $\betav_{k,0}$ are not determined by the values of $\alpha$
        \choice The derivative of loss with respect to the bias terms is always zero.
    \end{checkboxes}
    \clearpage
...
\part Adding a Hidden Layer
\begin{subparts}
\subpart[2] Now, try adding another hidden layer to your neural network. The hyperparameters for the model should be the same as the table above, except for learning rate. The \textbf{2 hidden layers} should both have a dimension of \textbf{50}, and your learning rate should be \textbf{0.003}. Run the optimization for 100 epochs. Remember that adding another hidden layer means you should add both a linear layer and a sigmoid layer in your code.
...
\part Understanding the Dataset
...
\textbf{Identify Misclassified Points:  }
   Use the script \texttt{<incorrect\_finder.py>} to locate the misclassified points in \texttt{<small\_train\_out.labels>} after training on  \texttt{<small\_train.csv>}. Running the command below will generate a file named \texttt{<incorrect\_label.csv>}, which lists the indices and details of each misclassified data point.

   Note that \texttt{<small\_train\_out.labels>} refers to the file generated by the training command provided in the handout.
   \begin{lstlisting}[language=Shell]
...
\begin{algorithm}
      \begin{algorithmic}[1]
        \Procedure{Forward}{$\av$}
          ...
          \State Cache intermediate value(s) for the backward pass \Comment{See Written Question 2.2(e)}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1] % The number tells where the line numbering should start
    \Procedure{NNBackward}{$\yv$, $\hat{\yv}$}
      ...
      \State $\gv_{\bv} = \textproc{dSoftmaxCrossEntropy}(\yv, \hat{\yv}, g_J)$ \Comment{See Written Question 2.2(c)}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1] % The number tells where the line numbering should start
    \Procedure{SGD}{Training data $\Dc_{train}$, \ntset\thinspace data $\Dc_t$} 
      ...
      \For{$e \in \{0, 1, \ldots, E-1\}$} \Comment{For each epoch}
